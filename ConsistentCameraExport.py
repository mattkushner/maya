...
def duplicateCameraAndBake(startFrame, endFrame, stepSize, targetCamera,
	...
	try:
		import pymel.core
		targetImagePlaneShape = pymel.core.getAttr("%s.imagePlane" % targetCamera)[0].getShape().name()
		mc.setAttr('%s.displayOnlyIfCurrent' % targetImagePlaneShape, True)
		mel.eval('AEchangeLookThroughCamera %s;' % targetImagePlaneShape)

		dupImagePlaneShape = pymel.core.getAttr("%s.imagePlane" % tempDup)[0].getShape().name()
...
class ConsistentCameraExportUI:
	...
	def __init__(self):
		...
		mc.optionMenuGrp(self.exportLayoutNuke, e=True, select=2)
	...
	def refresh_frame_range(self):
		minFrame, maxFrame = get_slider_range()
		mc.intFieldGrp(self.frameRange, e=True, value1=minFrame, value2=maxFrame)
	...
	def exportUILayout(self, *args):
		...
		layoutGroupName = mc.textFieldButtonGrp(
			self.selectLayoutGrp, q=True, text=True)
		trackedPointsFolder = mc.listRelatives(layoutGroupName, children=True, f=True)
		objsToExport = mc.listRelatives(trackedPointsFolder, children=True, f=True)
