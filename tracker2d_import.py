...
def closetsRayIntersect( fnMesh, accelParams, point, direction ):
    """
...
def createLocators( trackingData,
    ...
    for tracker in trackingData:
        loc, joint = createSingleLocator( tracker, cameraShape, locatorDisplayScale )
...
def createLocatorsAnimProjectCam( trackingData,
                                  ...
                                  locatorDisplayScale,
                                  targetMesh=None,
                                  sticksToLastHit=False ):

    # projection setup - currently rebuilt per frame bellow
    # build fnMesh and octree to pass to the closetsRayIntersect
    ...
    result_trackers = [ ]
    result_joints = [ ]
    previousHits = {} # Stores the hit info from the ray tracing.
    for tracker in trackingData:
        loc, joint = createSingleLocator( tracker, cameraShape, locatorDisplayScale )
        result_trackers.append( loc )
        result_joints.append( joint )
        previousHits[ tracker[0] ] = None

    # step through time
    for f in xrange( timeStart, timeEnd+1 ):
        # set frame
        ...
        #api.MScriptUtil().createMatrixFromList( worldMatrix, worldMMatrix )
        #camPosMVector = api.MVector( worldMMatrix(3,0), worldMMatrix(3,1), worldMMatrix(3,2) )
        ...
        for tracker in trackingData:
            ...
            if targetMesh:
                mc.normalConstraint( targetMesh, tracker_name, weight=1, aimVector=(0, 0, 1), upVector=(0, 1, 0), worldUpType='scene')
            ...
            for frameData in tracker[1]:
                ...
                if fnum == f:
                    ...
                    if targetMesh:
                        ...
                        # project!
                        hit, hitPos, hitInfo = closetsRayIntersect( fnMesh, accelParams, camPos, projDir )
                        if hit:
                            locPos = hitPos
                            previousHits[ tracker[0] ] = hitInfo
                            # print 'Inside locPos'
                            # print locPos
                            break
            ...
            if sticksToLastHit:
                if previousHits[ tracker[0] ] and locPos == None:
                    previousHit = previousHits[ tracker[0] ]

                    try:
                        snapPosition = fnMesh.getPointAtUV(previousHit[2], previousHit[0], previousHit[1], api.MSpace.kWorld, UV_SET_NAME)
                        locPos = snapPosition
                    except RuntimeError as e:
...
class   TrackerImportUI:
    ...
    def __init__(self):
        ...
        self.z = mc.floatSliderGrp( label='Z Depth: ', field=True, minValue=0.01, maxValue=100.0,
            fieldMinValue=0.001, fieldMaxValue=1000.0, value=10 )
        ...
        #     #fieldMinValue=1, fieldMaxValue=8192, value=2048 )
        #     fieldMinValue=1, fieldMaxValue=16384, value=defaultWidth )
        # self.resY = mc.intSliderGrp( label='Y Pixel Resolution: ', field=True, minValue=512, maxValue=2048,
        #     #fieldMinValue=1, fieldMaxValue=8192, value=1152 )
        #     fieldMinValue=1, fieldMaxValue=16384, value=defaultHeight)
        ...
        self.frameOffset = mc.intSliderGrp( label='Frame Offset: ', field=True, minValue=-100, maxValue=100,
            fieldMinValue=-100000, fieldMaxValue=100000, value=0 )

        self.displayScale = mc.floatSliderGrp( label='Locator Display Scale: ', field=True, minValue=0.1, maxValue=10.0,
            fieldMinValue=0.001, fieldMaxValue=100, precision=3, value=0.2 )

        self.animated = mc.checkBoxGrp( numberOfCheckBoxes=1, label='Animated Camera: ', onCommand=self.animatedOnCommand,
            offCommand=self.animatedOffCommand )

        self.projected = mc.checkBoxGrp( numberOfCheckBoxes=1, label='Project Points: ', onCommand=self.projectOnCommand,
            offCommand=self.projectOffCommand, enable=False )

        self.targetMesh = mc.textFieldButtonGrp( label='Target Mesh: ', text='', buttonLabel='...', bc=self.loadTargetMesh,
            enable=False )

        self.sticksToLastHit = mc.checkBoxGrp( numberOfCheckBoxes=1, label='Sticks To Last Hit: ', enable=False, v1=True )
        ...
        mc.formLayout( self.form,
            ...
            attachForm=[
                ...
                (self.sticksToLastHit, 'left', 2),
                (self.sticksToLastHit, 'right', 2),
                (self.debug, 'left', 2),
                (self.debug, 'right', 2),
            ...
            attachControl=[
                ...
                (self.animated, 'top', 2, self.displayScale),
                (self.projected, 'top', 2, self.animated),
                (self.targetMesh, 'top', 2, self.projected),
                (self.sticksToLastHit, 'top', 2, self.targetMesh),
                (self.debug, 'top', 2, self.sticksToLastHit),
    ...
    def projectOnCommand(self, *args):
        """ Project is ticked, so enable targetMesh and sticksToLastHit. """
        mc.textFieldButtonGrp(self.targetMesh, e=True, enable=True)
        mc.checkBoxGrp(self.sticksToLastHit, e=True, enable=True)

    def projectOffCommand(self, *args):
        """ Project is not ticked, so disable targetMesh. """
        mc.textFieldButtonGrp(self.targetMesh, e=True, enable=False)
        mc.checkBoxGrp(self.sticksToLastHit, e=True, enable=False)

    def animatedOnCommand(self, *args):
        ...
        if mc.checkBoxGrp(self.projected, q=True, v1=True):
            self.projectOnCommand()
        else:
            self.projectOffCommand()

    def animatedOffCommand(self, *args):
    ...
    def defaults(self, *args):
        mc.floatSliderGrp(self.z, e=True, v=10)
        # mc.intSliderGrp(self.resX, e=True, v=2048)
        # mc.intSliderGrp(self.resY, e=True, v=1152)
        mc.floatSliderGrp(self.offsetX, e=True, v=0.0)
        mc.floatSliderGrp(self.offsetY, e=True, v=0.0)
        mc.intSliderGrp(self.frameOffset, e=True, v=0)
        mc.floatSliderGrp(self.displayScale, e=True, v=0.2)
        mc.checkBoxGrp(self.animated, e=True, v1=False)
        mc.checkBoxGrp(self.projected, e=True, v1=False, enable=False)
        mc.textFieldButtonGrp(self.targetMesh, e=True, enable=False)
        mc.checkBoxGrp(self.sticksToLastHit, e=True, enable=False, v1=True)
    ...
    def execute(self, *args):
        ...
        projectionOn = mc.checkBoxGrp(self.projected, q=True, v1=True)
        ...
        if (debugMode):
            ...
            print( 'renderWidth: %d' % renderWidth )
            print( 'renderHeight: %d' % renderHeight )
        ...
        if not animatedCamera:
            trackers, joints = createLocators( trackingData, xneg, xpos, yneg, ypos, z, wm, cameraShape, camWorldPos, locatorDisplayScale )
        else:
            ...
            if not projectionOn:
                trackers, joints = createLocatorsAnimProjectCam( trackingData, z, cameraShape, locatorDisplayScale )
            else:
                ...
                mc.polyAutoProjection ( uv_geo, layoutMethod=0, projectBothDirections=0, insertBeforeDeformers=0, createNewMap=True, layout=2, scaleMode=1, optimize=1, planes=12, uvSetName=UV_SET_NAME, percentageSpace=0.2, worldSpace=0)
                ...
                mc.Unfold3D( TEMP_UV_GEO_NAME, unfold=True, iterations=1, pack=True, borderintersection=True, triangleflip=True, mapsize=1024, roomspace=2)
                ...
                sticksToLastHit = mc.checkBoxGrp(self.sticksToLastHit, q=True, v1=True)
                trackers, joints = createLocatorsAnimProjectCam( trackingData, z, cameraShape, locatorDisplayScale, targetMesh, sticksToLastHit)

                # Delete the temporary uv mesh
                mc.delete ( uv_geo )
                mc.delete ( transfer_node )
                # Delete the temporary UV set
                #mc.polyUVSet (targetMesh, delete=True, uvSet=UV_SET_NAME)
        ...
        tracker_group = mc.group( empty=True, name='tracker_group')
        ...
        joint_group = mc.group( empty=True, name='joint_group')
        # Add each tracker to the group
        for joint in joints:
